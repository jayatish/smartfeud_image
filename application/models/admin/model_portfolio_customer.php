...
class Model_portfolio_customer extends CI_model
{
...
//------------------------- Start function to update status in users --------------------------------//
	function update_status_customer()
	{
		...
		$update=$this->db->update('customer',$update_status);
		if($update)
		{
...
//----------------------------- Start function to delete cms details -------------------------------//
	function delete_customer()
	{
		...
		$sql_del = "select * from customer where cu_id = '".$edit_id."' ";
	...
	function update_customer()
	{
        ...
        $sql_del = "select * from customer where cu_id = '".$id."' ";
        $query_del = $this->db->query($sql_del);
		...
		if ($_FILES['c_image']['size'] != 0 && $_FILES['c_image']['name'] != "")
        {
			...
			$configUpload['upload_path'] = $this->config->item('file_upload_absolute_path').'uploads/customer_image/';
			$configUpload['allowed_types'] = 'gif|jpg|png';
			$configUpload['encrypt_name']	= 'TRUE';
			$this->upload->initialize($configUpload);
	        $this->upload->do_upload('c_image');
			$upload_data = $this->upload->data(); //Returns array of containing all of the data related to the file you uploaded.
			$filepath_image = 'uploads/customer_image/'.$upload_data['file_name'];

			$config = array(
    						  'source_image'  => $upload_data['full_path'], //path to the uploaded image
    						  'new_image'     => $this->config->item('file_upload_absolute_path').'uploads/customer_image/thumb/', //path to
    							'maintain_ratio'    => true,
							    'create_thumb'       => true,   
   								 'width'             => 100,
    							'height'            => 101
                            ...
                            $filepath_image_thumb = 'uploads/customer_image/thumb/'.$thumbfilename;
		...
		$this->db->where("customer.cu_id",$this->input->post('edit_hidden'));
