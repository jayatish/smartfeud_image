...
//------------------------- Start function to get the cms list -------------------------------//
	function GetReportList($report_type_result,$date_range_result,$date_range_part1_result,$from_date_result,$to_date_result)
	{
		...
		if(($report_type_result==1) || ($report_type_result==2)){
			...
			if($report_type_result==1){
				...
				}if($date_range_result==2){
					$from_date_range = date('Y-m-d',strtotime($from_date_result));
					$to_date_range = date('Y-m-d',strtotime($to_date_result));
					$this->db->where("(add_date >= '".$from_date_range." 00:00:00' and add_date <= '".$to_date_range." 23:59:59')");
				}
				$this->db->order_by('add_date','DESC');
				$q = $this->db->get('osc_ticket');
				//echo $this->db->last_query();
...
//---------------------------- End function to get the count of Particuler Ticket --------------------------------//
	function TicketMonthlyCount($date,$status_id,$date_range_result,$date_range_part1_result,$from_date_result,$to_date_result){
		$data = 0;
		$current_month = '-'.date('m').'-';
		if($date_range_result==1){
			if($date_range_part1_result==1){
				$today_date = $date.'-'.date('d');
				$this->db->like('add_date',$today_date,'after');
			}if($date_range_part1_result==2){
				$previous_day = $date.'-'.date('d',mktime(date('H'),date('i'),date('s'),date('m'),date('d')-1,date('Y')));
				$this->db->like('add_date',$previous_day,'after');
			}/*if($date_range_part1_result==3){
				$first_day_this_month = date('Y-m-01');
				$last_day_this_month  = date('Y-m-t');
			...
			if($date_range_part1_result==6){
				$monday = date( 'Y-m-d', strtotime( 'monday this week' ) );
				$sunday = date( 'Y-m-d', strtotime( 'sunday this week' ) );
				if((strpos($sunday, $current_month)!=false) && (strpos($monday, $current_month)!=false)){
					$this->db->where("(add_date >= '".$monday." 00:00:00' and add_date <= '".$sunday." 23:59:59')");
				}else{
					if(strpos($sunday, $current_month)==false){
						$last_day_this_month  = date('Y-m-t');
						$this->db->where("(add_date >= '".$monday." 00:00:00' and add_date <= '".$last_day_this_month." 23:59:59')");
					}if(strpos($monday, $current_month)==false){
						$first_day_this_month = date('Y-m-01');
						$this->db->where("(add_date >= '".$first_day_this_month." 00:00:00' and add_date <= '".$sunday." 23:59:59')");
					}
				...
				$monday = date( 'Y-m-d', strtotime( 'monday this week' ) );
				$friday = date( 'Y-m-d', strtotime( 'friday this week' ) );
				if((strpos($friday, $current_month)!=false) && (strpos($monday, $current_month)!=false)){
					$this->db->where("(add_date >= '".$monday." 00:00:00' and add_date <= '".$friday." 23:59:59')");
				}else{
					if(strpos($friday, $current_month)==false){
						$last_day_this_month  = date('Y-m-t');
						$this->db->where("(add_date >= '".$monday." 00:00:00' and add_date <= '".$last_day_this_month." 23:59:59')");
					}if(strpos($monday, $current_month)==false){
						$first_day_this_month = date('Y-m-01');
						$this->db->where("(add_date >= '".$first_day_this_month." 00:00:00' and add_date <= '".$friday." 23:59:59')");
					}
				}
			}if($date_range_part1_result==9){
				$monday = date( 'Y-m-d', strtotime( 'monday last week' ) );
		...
		if($date_range_result==2){
			$from_date = date( 'Y-m-d', strtotime($from_date_result) );
			$to_date = date( 'Y-m-d', strtotime($to_date_result) );
			if((strpos($from_date, $current_month)!=false) && (strpos($to_date, $current_month)!=false)){
				$this->db->where("(add_date >= '".$from_date." 00:00:00' and add_date <= '".$to_date." 23:59:59')");
			}else{
				if(strpos($to_date, $current_month)==false){
					$last_day_this_month  = date('Y-m-t');
					$this->db->where("(add_date >= '".$from_date." 00:00:00' and add_date <= '".$last_day_this_month." 23:59:59')");
				}if(strpos($from_date, $current_month)==false){
					$first_day_this_month = date('Y-m-01');
					$this->db->where("(add_date >= '".$first_day_this_month." 00:00:00' and add_date <= '".$to_date." 23:59:59')");
				}
				
			}
		}
		//echo $date."===".$status_id."===".$date_range_result."===".$date_range_part1_result;
...
//---------------------------- Start function to get the count of Assigned Tickets --------------------------------//
	function AssignedCategoryTicketCount($user_id,$date_range_result,$date_range_part1_result,$from_date_result,$to_date_result){
		$data = 0;
		if($date_range_result==1){
			$monday = date( 'Y-m-d', strtotime( 'monday this week' ) );
			$friday = date( 'Y-m-d', strtotime( 'friday this week' ) );
			...
			$form_date = date('Y-m-d',strtotime($from_date_result));
			$to_date = date('Y-m-d',strtotime($to_date_result));
			$this->db->where("(add_date >= '".$form_date." 00:00:00' and add_date <= '".$to_date." 23:59:59')");
		}
...
//------------------------------ Start function to get the counts of user for assigned tickets -------------------------------------//
	function AssignedUserTicketCount($user_id,$date_range_result,$date_range_part1_result,$from_date_result,$to_date_result){
		$data = 0;
		if($date_range_result==1){
			$monday = date( 'Y-m-d', strtotime( 'monday this week' ) );
			$friday = date( 'Y-m-d', strtotime( 'friday this week' ) );
			if($date_range_part1_result==1){
				$today_date = date('Y-m-d');
				$this->db->like('assign_date',$today_date,'after');
			}if($date_range_part1_result==2){
				$previous_day = date('Y-m-d',mktime(date('H'),date('i'),date('s'),date('m'),date('d')-1,date('Y')));
				$this->db->like('assign_date',$previous_day,'after');
			}if($date_range_part1_result==3){
				$first_day_this_month = date('Y-m-01');
				$last_day_this_month  = date('Y-m-t');
				$this->db->where("(assign_date >= '".$first_day_this_month." 00:00:00' and assign_date <= '".$last_day_this_month." 23:59:59')");
			}if($date_range_part1_result==4){
				$first_day_last_month = date('Y-m-d', strtotime('first day of last month'));
				$last_day_last_month  = date('Y-m-d', strtotime('last day of last month'));
				$this->db->where("(assign_date <= '".$first_day_last_month." 00:00:00' and assign_date >= '".$last_day_last_month." 23:59:59')");
			}if($date_range_part1_result==5){
				$first_day = date('Y-m-d');
				$last_day  = date('Y-m-d', mktime(date('H'),date('i'),date('s'),date('m'),date('d')-30,date('Y')));
				$this->db->where("(assign_date <= '".$first_day." 00:00:00' and assign_date >= '".$last_day." 23:59:59')");
			}if($date_range_part1_result==6){
				$monday = date( 'Y-m-d', strtotime( 'monday this week' ) );
				$sunday = date( 'Y-m-d', strtotime( 'sunday this week' ) );
				$this->db->where("(assign_date >= '".$monday." 00:00:00' and assign_date <= '".$sunday." 23:59:59')");
			}if($date_range_part1_result==7){
				$monday = date( 'Y-m-d', strtotime( 'monday last week' ) );
				$sunday = date( 'Y-m-d', strtotime( 'sunday last week' ) );
				$this->db->where("(assign_date >= '".$monday." 00:00:00' and assign_date <= '".$sunday." 23:59:59')");
				...
				$monday = date( 'Y-m-d', strtotime( 'monday this week' ) );
				$friday = date( 'Y-m-d', strtotime( 'friday this week' ) );
				$this->db->where("(assign_date >= '".$monday." 00:00:00' and assign_date <= '".$friday." 23:59:59')");
			}if($date_range_part1_result==9){
				$monday = date( 'Y-m-d', strtotime( 'monday last week' ) );
				$friday = date( 'Y-m-d', strtotime( 'friday last week' ) );
				$this->db->where("(assign_date >= '".$monday." 00:00:00' and assign_date <= '".$friday." 23:59:59')");
				...
				$current_year = date( 'Y');
				$this->db->like('assign_date',$current_year,'after');
			}if($date_range_part1_result==11){
				$last_year = date( 'Y',mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('Y')-1));
				$this->db->like('assign_date',$last_year,'after');
			}
		}if($date_range_result==2){
			$form_date = date('Y-m-d',strtotime($from_date_result));
			$to_date = date('Y-m-d',strtotime($to_date_result));
			$this->db->where("(assign_date >= '".$form_date." 00:00:00' and assign_date <= '".$to_date." 23:59:59')");
		}
...
//--------------------------------- End function to get the counts of user for assigned tickets --------------------------------------// 
//------------------------------- Start function to get the count of Replies tickets --------------------------------------//
	function TicketRepliedCount($user_id,$date_range_result,$date_range_part1_result,$from_date_result,$to_date_result){
		$data = 0;
		if($date_range_result==1){
			$monday = date( 'Y-m-d', strtotime( 'monday this week' ) );
			$friday = date( 'Y-m-d', strtotime( 'friday this week' ) );
...
//-------------------------------- End function to get the count of Replies tickets --------------------------------------//
//----------------------------------- Start function to get the count of category based reply --------------------------------------//
	function CategoryBasedReplyCount($reply_type,$category_id,$date_range_result,$date_range_part1_result,$from_date_result,$to_date_result){
		$data = 0;
		$this->db->select('osc_ticket_notes.*');
		$this->db->from('osc_ticket');
		$this->db->from('osc_ticket_notes');
		$this->db->where('osc_ticket.company_id',$category_id);
		if($date_range_result==1){
			$monday = date( 'Y-m-d', strtotime( 'monday this week' ) );
			$friday = date( 'Y-m-d', strtotime( 'friday this week' ) );
			if($date_range_part1_result==1){
				$today_date = date('Y-m-d');
				$this->db->like('osc_ticket.add_date',$today_date,'after');
			}if($date_range_part1_result==2){
				$previous_day = date('Y-m-d',mktime(date('H'),date('i'),date('s'),date('m'),date('d')-1,date('Y')));
				$this->db->like('osc_ticket.add_date',$previous_day,'after');
			}if($date_range_part1_result==3){
				$first_day_this_month = date('Y-m-01');
				$last_day_this_month  = date('Y-m-t');
				$this->db->where("(osc_ticket.add_date >= '".$first_day_this_month." 00:00:00' and osc_ticket.add_date <= '".$last_day_this_month." 23:59:59')");
			}if($date_range_part1_result==4){
				$first_day_last_month = date('Y-m-d', strtotime('first day of last month'));
				$last_day_last_month  = date('Y-m-d', strtotime('last day of last month'));
				$this->db->where("(osc_ticket.add_date <= '".$first_day_last_month." 00:00:00' and osc_ticket.add_date >= '".$last_day_last_month." 23:59:59')");
			}if($date_range_part1_result==5){
				$first_day = date('Y-m-d');
				$last_day  = date('Y-m-d', mktime(date('H'),date('i'),date('s'),date('m'),date('d')-30,date('Y')));
				$this->db->where("(osc_ticket.add_date <= '".$first_day." 00:00:00' and osc_ticket.add_date >= '".$last_day." 23:59:59')");
			}if($date_range_part1_result==6){
				$monday = date( 'Y-m-d', strtotime( 'monday this week' ) );
				$sunday = date( 'Y-m-d', strtotime( 'sunday this week' ) );
				$this->db->where("(osc_ticket.add_date >= '".$monday." 00:00:00' and osc_ticket.add_date <= '".$sunday." 23:59:59')");
			}if($date_range_part1_result==7){
				$monday = date( 'Y-m-d', strtotime( 'monday last week' ) );
				$sunday = date( 'Y-m-d', strtotime( 'sunday last week' ) );
				$this->db->where("(osc_ticket.add_date >= '".$monday." 00:00:00' and osc_ticket.add_date <= '".$sunday." 23:59:59')");
				...
				$monday = date( 'Y-m-d', strtotime( 'monday this week' ) );
				$friday = date( 'Y-m-d', strtotime( 'friday this week' ) );
				$this->db->where("(osc_ticket.add_date >= '".$monday." 00:00:00' and osc_ticket.add_date <= '".$friday." 23:59:59')");
			}if($date_range_part1_result==9){
				$monday = date( 'Y-m-d', strtotime( 'monday last week' ) );
				$friday = date( 'Y-m-d', strtotime( 'friday last week' ) );
				$this->db->where("(osc_ticket.add_date >= '".$monday." 00:00:00' and osc_ticket.add_date <= '".$friday." 23:59:59')");
				...
				$last_year = date( 'Y',mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('Y')-1));
				$this->db->like('osc_ticket.add_date',$last_year,'after');
			}
		}if($date_range_result==2){
			$form_date = date('Y-m-d',strtotime($from_date_result));
			$to_date = date('Y-m-d',strtotime($to_date_result));
			$this->db->where("(osc_ticket.add_date >= '".$form_date." 00:00:00' and osc_ticket.add_date <= '".$to_date." 23:59:59')");
		}
		$this->db->where('osc_ticket.id = osc_ticket_notes.ticket_id');
		if($reply_type=='staff'){
			$this->db->where('osc_ticket_notes.type','0');
		}
		$q = $this->db->get();
		return $q->num_rows;

	}
//------------------------------------ End function to get the count of category based reply ---------------------------------------//
